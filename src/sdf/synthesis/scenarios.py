...
def _apply(spec, tweaks: Dict):
    ...
    if "daily_orders_per_a_sku_mult" in tweaks:
        kw["daily_orders_per_a_sku"] = round(
            spec.daily_orders_per_a_sku * tweaks["daily_orders_per_a_sku_mult"], 3)
...
def run_scenarios(base_spec=None, names: List[str] = None,
    ...
    for r in rows:
        r["safety_stock_vs_baseline_pct"] = (
            round(100 * (r["safety_stock_units"] - base_row["safety_stock_units"])
                  / max(1, base_row["safety_stock_units"]), 1))
