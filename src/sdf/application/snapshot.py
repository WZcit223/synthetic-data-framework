...
_BACKTEST_KEYS = ("model", "MAE", "RMSE", "MAPE_pct", "WAPE_pct", "bias")
...
def _backtest_table(bt: dict) -> list[str]:
    ...
    return lines + _table(
        ...
        [[r["model"], r["MAE"], r["RMSE"], r["MAPE_pct"], r["WAPE_pct"], r["bias"]] for r in bt["results"]],
    )
...
def render_markdown(snap: dict) -> str:
    ...
    lines = [
        ...
        "",
        "MAPE % averages |error| / actual over the test points whose actual is positive; "
        "WAPE % is Σ|error| / Σactual over all test points (see `analytics/metrics.py`). "
        "Models are ranked by MAE.",
        "",
    ]
