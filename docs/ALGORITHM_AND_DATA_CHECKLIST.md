...
| Synthesis fidelity (B1) | **measured**: baseline 78/100 **+ Gaussian-copula/SDMetrics 0.92** + **a detection test in every table evaluation** (B4) and column kinds for table synthesizers + **a Bayesian-network table synthesizer** (detection AUC 0.49 on the sample, 0.53 on the real extract) | CTGAN/TVAE; evaluation on a user's own table; membership inference (B3) |
| Synthetic utility (B2) | **measured** TSTR ratio 0.90–1.00 (as useful as real) | rerun on full dataset + learned models |
| Replenishment (C2) | **(s,S) policy** + service-level selector + replayed comparison with a no-safety-stock policy + **a cost-based policy choosing both levels per SKU, scored out of sample** | fitted lead-time demand and stochastic lead times |
...
| Anomaly detection (C3) | **seasonal-residual + robust-z** on demand, live + **detector plug-ins over demand, stock and receipts per SKU, scored on injected anomalies** (an Isolation Forest finds stock shrinkage) | an autoencoder over real multi-sensor history |
...
| B4 | Detection test | "synthetic-vs-real" classifier AUC → 0.5 is ideal (**done**: gradient-boosted classifier, 5-fold, in every table evaluation; `docs/VALIDATION.md` "Detection test") | mixed real+synthetic; a real holdout set |
...
| C2 | Replenishment | (s,S) service-level policy (normal approximation), and a cost-based (s,S) policy searched per SKU on its replayed cost, scored on held-out days | **cost-based (s,S) on a forecast + a fitted lead-time distribution** | supplier lead times, holding/stockout costs |
| C3 | Anomaly detection | a seasonal rule per SKU and an Isolation Forest over demand, stock and receipts, scored on injected anomalies | **an autoencoder** over real multivariate history | historical normal-operations data |
...
|---|---|---|---|
| A1 | Tabular synthesis (SKU / inventory) | `sdf/synthesis/bayes_net.py`<br>`sdf/synthesis/bootstrap.py::BootstrapTable`<br>`sdf/synthesis/sdv_synth.py`<br>`sdf/synthesis/warehouse.py::WarehouseGenerator._gen_inventory` | — |
...
| B4 | Detection test | `sdf/cli.py::cmd_privacy`<br>`sdf/validation/detection.py` | — |
...
| C2 | Replenishment | `sdf/analytics/demand.py::DemandProfile.variability`<br>`sdf/application/economics.py::financial_impact`<br>`sdf/simulation/outcome.py::SimulatedCost`<br>`sdf/simulation/policy.py::CostBasedPolicy`<br>`sdf/simulation/policy.py::ServiceLevelPolicy` | `sdf/application/economics.py`<br>`sdf/application/economics.py::financial_impact`<br>`sdf/cli.py::cmd_impact`<br>`sdf/simulation/outcome.py::CostModel` |
| C3 | Anomaly detection | `sdf/analytics/anomaly.py`<br>`sdf/analytics/detectors/builtin.py::IsolationForestDetector`<br>`sdf/application/anomaly_rules.py::rule_anomalies` | — |
| C4 | Slotting / layout | — | — |
