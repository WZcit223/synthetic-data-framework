...
| Demand forecast | baseline **+ real-data backtest** + **AR/seasonal model** (C1) + **forecaster plug-ins with intervals, backtested per SKU** against the exact distribution of a demand benchmark | gradient boosting and DeepAR/TFT; full dataset to beat `snaive` |
...
|---|----------|----------------------|------------------|-------------|
| C1 | Demand forecast | avg daily demand; per-SKU baselines with empirical-error intervals, scored by a probabilistic backtest | **DeepAR / Temporal Fusion Transformer / LightGBM** | dated order history + calendar/promo features |
...
| B4 | Detection test | — | — |
| C1 | Demand forecast | `sdf/analytics/forecast.py`<br>`sdf/analytics/forecasters/builtin.py`<br>`sdf/analytics/models.py`<br>`sdf/application/replenishment.py::demand_series`<br>`sdf/cli.py::cmd_backtest` | `sdf/simulation/benchmark.py::DemandBenchmark` |
