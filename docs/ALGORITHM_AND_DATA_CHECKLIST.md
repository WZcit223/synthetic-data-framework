...
| Synthesis fidelity (B1) | **measured**: baseline 78/100 **+ Gaussian-copula/SDMetrics 0.92** + **a detection test in every table evaluation** (B4; 0.55 on the sample, 0.89 on the real extract) and column kinds for table synthesizers | a synthesizer that learns the joint distribution (CTGAN/TVAE or a Bayesian network); membership inference (B3) |
