...
5. **Measured numbers are honest.** `docs/VALIDATION.md` records what the code
   produced, including where a model did not win. Its reproducible numbers are a
   block generated by `uv run sdf validate --update-doc docs/VALIDATION.md`
   from the same snapshot `golden_test.py` asserts, and a test fails when the
...
CI (`.github/workflows/ci.yml`) runs these on Python 3.12, 3.13 and 3.14 for
