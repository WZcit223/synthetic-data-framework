...
test("an input the server would refuse is refused with a reason", () => {
  ...
  assert.equal(readParam(seed, "12345678901234567890").error, "seed: between -9007199254740991 and 9007199254740991"); // would lose digits in JSON
  assert.deepEqual(readParam(seed, "999999999999999"), { value: 999999999999999 });
